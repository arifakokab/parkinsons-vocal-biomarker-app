...
This initiative provides free, accessible, non-invasive screening and severity estimation of Parkinson's Disease via voice, for public use.**
---
""")
...
# --- 4. AUDIO RECORDING (Manual Start/Stop) ---
...
# Initialize session state for recording logic
if "recording" not in st.session_state:
    st.session_state.recording = False
if "audio_ctx" not in st.session_state:
    st.session_state.audio_ctx = None
...
with col1:
    if not st.session_state.recording:
        if st.button("Start Recording"):
            ...
            st.session_state.frames = []  # Reset previous frames
with col2:
    if st.session_state.recording:
        if st.button("Stop & Save Audio"):
            st.session_state.recording = False

# Streamlit-webrtc: Start only when "Start Recording" pressed
audio_ctx = None
if st.session_state.recording:
    class AudioRecorder(AudioProcessorBase):
        ...
        def recv(self, frame: av.AudioFrame):
            ...
            st.session_state.frames.append(frame.to_ndarray().flatten())
            return frame
...
# Process and save audio on stop
if not st.session_state.recording and st.session_state.frames:
    # Combine all recorded audio frames into one array
    all_audio = np.concatenate(st.session_state.frames)
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as f:
        sf.write(f, all_audio, 48000)
        st.session_state.audio_bytes = f.name
    st.session_state.frames = []  # Reset after saving
...
if st.button("Run Analysis", type="primary"):
    audio_bytes = st.session_state.audio_bytes  # Always get from session
    if not st.session_state.get("audio_saved", False) or not audio_bytes:
